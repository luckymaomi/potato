...
- 运行反馈：顶部显示当前运行范围的真实完成数和当前阶段；只有供应商明确返回百分比时，顶部、节点与检查器才显示百分比，不按轮询次数或固定节点伪造进度
...
- “运行整个画布”按完整显式 DAG 拓扑排序并执行全部节点；“运行未完成”只执行缺失或失效节点；单节点只执行自己。三种范围共用同一执行主干，并且只追加当次执行时已经完成的直接上游结果
- 画布内提交整集合成
- Everything 持久日志：默认写入 `backend/logs/everything.log`，覆盖 HTTP、CRUD、画布、生产命令、任务、供应商、本地归档、合成和验收脚本，并自动脱敏凭据与二进制内容
- Agnes《雨夜外卖》预写 Demo v15：36 个节点、56 条显式数据边和 1 个工作流组；镜头描述直接属于分镜图/视频节点
...
初始化成功后启动应用，打开《雨夜外卖》，由你在画布顶部选择“运行未完成”或“运行整个画布”。图片使用 Agnes `agnes-image-2.5-flash`，视频使用 Agnes `agnes-video-2.5-flash`。
