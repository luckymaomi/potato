...
| 阶段 | 做什么 |
| --- | --- |
| **总览与剧本** | 编辑项目画风锁（基调、参考设定）与合一的故事/剧本文本；给人读与写作，不自动注入分镜配方；不建镜头、不自动建镜 |
| **项目资产库** | 建角色 / 场景 / 道具卡，组装或手写产出提示词，逐张生成或上传标准资产图 |
| **分镜台** | 独立建镜；写本镜画面，勾选出场资产，上传其他参考图；编辑后直接生成或上传底板，也可先组装预览配方 |

- **双通道**：分镜组装使用资产卡结构化文本与标准图；资产图生成使用用户保存的产出提示词与输入参考图。预设切换与页面刷新不会覆盖已保存的最终文本。
- **手选生成**：在 AI 配置页保存图片模型预设；在分镜台选择画幅。缺项时入口禁用并给出原因。
...
配置 `config.yaml` 后，打开「AI 配置」刷新模型目录，选择图片默认预设，再到分镜台出底板。
