...
土豆短剧把短剧制作收成一条可执行工作流：先写清故事与剧本，再建可复用的项目资产卡并产出标准资产图，再在分镜台组装图片与视频配方，逐镜出图出视频，最后合成整集。

项目之间相互隔离。同一项目内的角色卡、场景卡、道具卡可按 ID 跨集复用。故事、剧本、资产卡与分镜规格由用户编辑与保存；生成只消费已保存的最终文本与引用快照。

## 当前能力

| 阶段 | 做什么 |
| --- | --- |
| **总览与剧本** | 编辑项目钩子、世界观、主线与本集结构、场次节拍和剧本文本 |
| **项目资产库** | 建角色 / 场景 / 道具卡，组装或手写产出提示词，逐张生成或上传标准资产图 |
| **分镜台** | 填镜头规格、选本镜资产与额外参考图，组装并保存图片 / 视频配方，出分镜图与单镜视频，再合成整集 |

- **双通道**：分镜组装使用资产卡结构化文本与标准图；资产图生成使用用户保存的产出提示词与输入参考图。预设切换与页面刷新不会覆盖已保存的最终文本。
- **手选生成**：在 AI 配置页保存图片 / 视频模型预设；在制作页选择画幅，并在目录声明档位时选择视频时长。缺项时入口禁用并给出原因。
- **媒体与归档**：任务提交后返回本地任务 ID 并轮询；成功媒体归档后，业务对象指向已完成且可用的 generation。分镜台支持下载当前成片，以及按整集或选中镜头导出镜头包。
...
配置 `config.yaml` 后，打开「AI 配置」刷新模型目录，选择图片 / 视频默认预设，再到制作页出图出视频。
...
成片 / 镜头包交付、生成门闸、模型能力与复核规则见 `spec.md`。
