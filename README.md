...
人工剧本 → 项目资产卡与标准资产图 → 分格台底板 → 页组装拼页/字层/配音 → 合成页与话数包
...
土豆漫画把短剧素材收成一条可执行漫画工作流：先写清故事与剧本，再建可复用的项目资产卡并产出标准资产图，在分格台逐格组装并归档底板，最后在独立页组装完成拼页、气泡、配音、页图导出和话数包。
...
| **分格台** | 填漫画格规格（节拍、动作定格、表情、取景、构图、视角、光线、氛围），选本格资产与额外参考图，组装并保存图片配方，手选画幅后生成或上传单格底板，查看历史和待复核；对白与声音在页组装处理 |
| **页组装** | 按槽位选底板，拼单格 / 2×2 / 竖四格，拖拽缩放气泡，按格配音，导出页图并下载含底板的话数包 |
| **TTS 配置** | 独立保存 TTS 适配器、地址和凭据；音频完成后归档到本地 |

- **双通道**：分格组装使用资产卡结构化文本与标准图；资产图生成使用用户保存的产出提示词与输入参考图。预设切换与页面刷新不会覆盖已保存的最终文本。
- **手选生成**：在 AI 配置页保存图片模型预设；在分格台选择画幅。缺项时入口禁用并给出原因。
- **媒体与归档**：任务提交后返回本地任务 ID 并轮询；成功媒体归档后，业务对象指向已完成且可用的 generation。页组装导出页图，并下载包含底板、字层、阅读序、页信息、历史页图和本地音频的话数包。
...
配置 `config.yaml` 后，打开「AI 配置」刷新模型目录，选择图片默认预设，再到分格台出底板；在「TTS 配置」设置配音适配器后进入页组装。
...
分格门闸、页组装、字层、TTS、话数包与模型能力规则见 `spec.md`。
