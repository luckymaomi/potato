...
文本、图片和视频生成请求不设置应用、Undici 响应头或响应体超时；取得异步任务 ID 后也没有本地累计轮询截止线，只在供应商明确成功、明确失败或用户主动停止时结束。PearAPI 图片固定异步提交；图片和视频状态查询的暂时性网络错误会继续等待并写 `provider.image.poll.retry` / `provider.video.poll.retry`。模型目录等非生成查询仍保留短超时。初次生成提交默认不重发，避免无法确认供应商是否已受理时产生重复任务。
...
API Key、供应商地址和运行设置只读取仓库根目录私有 `config.yaml`；模型选择在 AI 配置页完成并保存到 SQLite，SQLite 不保存密钥。没有真实凭据时，确定性测试只证明请求合同和内部编排，不证明外部生成可用。
