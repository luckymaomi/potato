...
工作区 API 包含人工剧本、项目级角色/场景/道具资产卡、分镜、单镜图片/视频生成和整集合成。资产标准图直接使用卡片已保存的 `output_prompt`；建卡初始模板与分镜配方由后端提供。健康检查为 `GET /health`，任务状态为 `GET /api/v1/tasks/:id`，取消任务为 `POST /api/v1/tasks/:id/cancel`。数据与归档合同见根目录 `spec.md`。
