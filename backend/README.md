...
工作区 API 包含人工剧本、项目级角色/场景/道具资产卡、分镜、单镜图片/视频生成和整集合成。资产提示词与分镜双配方都只由显式组装接口编译；组装结果由用户查看、编辑并保存。资产、分镜图片和分镜视频生成分别直接使用已保存的最终提示词及对应图片引用，不在提交生成时重新组装。健康检查为 `GET /health`，任务状态为 `GET /api/v1/tasks/:id`，取消任务为 `POST /api/v1/tasks/:id/cancel`。数据与归档合同见根目录 `spec.md`。
