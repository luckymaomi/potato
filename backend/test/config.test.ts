...
test('显式配置路径可隔离数据库与本地媒体目录', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tomato-ai-drama-config-'));
  ...
  const previous = process.env.TOMATO_CONFIG_PATH;
  process.env.TOMATO_CONFIG_PATH = configPath;
  try {
    ...
    if (previous === undefined) delete process.env.TOMATO_CONFIG_PATH;
    else process.env.TOMATO_CONFIG_PATH = previous;
