...
export function StoryboardWorkspace() {
  ...
  const selectedAssets = assets.filter((asset) => selected?.project_asset_ids?.includes(asset.id))
    ...
    <div className="workspace-column director-room">
      ...
      {!items.length ? (
        ...
        <div className="director-board">
          <aside className="director-shot-rail" aria-label="镜头带">
            ...
            <Button type="dashed" block icon={<PlusOutlined />} onClick={() => void create()}>在末尾加入镜头</Button>
          </aside>

          <main className="director-stage">
            {selected ? <>
              <div className="director-stage-toolbar">
                <div><span className="stage-index">镜头 {String(selected.storyboard_number).padStart(2, '0')}</span><strong>{selected.title || '未命名镜头'}</strong><span className="stage-muted">{selected.shot_size || '景别待定'} · {selected.duration ?? 0}s</span></div>
              ...
              <div className="director-asset-tray">
                <div className="director-tray-heading"><div><strong>本镜素材</strong><span>当前镜头引用</span></div><Tag>{selectedAssets.length} 项</Tag></div>
                {selectedAssets.length ? <div className="director-tray-items">{selectedAssets.map((asset) => <div className="director-tray-item" key={asset.id}>{asset.image_url ? <img src={mediaUrl(asset.image_url)} alt={asset.name} /> : <div className="tray-item-placeholder">{assetLabels[asset.kind].slice(0, 1)}</div>}<div><strong>{asset.name}</strong><span>{assetLabels[asset.kind]}</span></div></div>)}</div> : <div className="director-tray-empty">还没有引用素材</div>}
              </div>
          ...
          <aside className="director-inspector">
            {selected ? <Form form={form} layout="vertical" className="director-form">
              ...
              <Collapse bordered={false} defaultActiveKey={['story', 'camera', 'assets']} expandIconPosition="end">
                ...
                <Collapse.Panel key="camera" header="镜头语言">
                  ...
                  <Form.Item name="duration" label="时长（秒）"><InputNumber min={0} precision={1} style={{ width: '100%' }} /></Form.Item>
                </Collapse.Panel>
                <Collapse.Panel key="assets" header="素材">
                  <Typography.Paragraph type="secondary" className="director-help-copy">选择要带入这一镜的人物、场景和道具。</Typography.Paragraph>
                  <Form.Item label="人物" name="character_asset_ids"><Select mode="multiple" allowClear options={assetOptions('character', assets)} placeholder="选择人物" /></Form.Item>
                  <Form.Item label="场景" name="scene_asset_ids"><Select mode="multiple" allowClear options={assetOptions('scene', assets)} placeholder="选择场景" /></Form.Item>
                  <Form.Item label="道具" name="prop_asset_ids"><Select mode="multiple" allowClear options={assetOptions('prop', assets)} placeholder="选择道具" /></Form.Item>
                </Collapse.Panel>
...
function ShotCard({ item, active, onClick }: { item: Storyboard; active: boolean; onClick: () => void }) {
  return <button type="button" className={`director-shot-card${active ? ' is-active' : ''}`} onClick={onClick}>
    ...
    <div className="director-shot-copy"><div><strong>{String(item.storyboard_number).padStart(2, '0')}</strong><span>{item.title || '未命名镜头'}</span></div><small>{item.shot_size || '景别待定'} · {item.duration ?? 0}s</small>{item.dialogue && <em>“{item.dialogue}”</em>}</div>
  </button>
}
...
function assetOptions(kind: AssetKind, assets: ProjectAsset[]) {
  return assets.filter((item) => item.kind === kind).map((item) => ({ value: item.id, label: item.name }))
}
