...
## 后续补充：参考图 Demo 与本地数据脚本

- [x] 按参考图重写 Demo 为《红女王》成人向宫廷复仇悬疑短剧，成年女性红女王为主角，5 个镜头。
- [x] 新增根目录 `init_demo.py`，调用当前后端 Demo 初始化入口。
- [x] 新增根目录 `clear_database.py`，无参数删除仓库相对路径 `backend/data` 整个目录。
- [x] README、history、Demo 打开提示和 Demo 回归断言同步。
- [x] Python 语法、后端完整测试和 `git diff --check` 验证通过。
- [ ] 本轮未执行 `clear_database.py`、commit、push 或真实供应商生成。

## 后续补充：三个媒体工作区逐项运行未完成

### 需求与当前事实

- 资产图、分镜图和镜头视频目前都支持单项生成与停止，但没有统一的“生成未完成”顺序控制。
- `useGenerationTracker` 已能恢复任务、轮询进度和取消当前任务；三个工作区可复用同一套任务状态。
- 视频生成成本更高，必须与分镜图生成分开提供入口；视频队列只处理已有分镜图但尚无视频的镜头。

### 目标与边界

- 在资产图页增加“生成未完成资产图”，按列表顺序一次提交一项，完成、失败或取消后再进入下一项。
- 在分镜台增加“生成未完成分镜图”，同样逐项运行；失败自动跳过并继续。
- 在生产页增加独立的“生成未完成视频”，只处理有分镜图且没有视频的镜头，支持停止当前任务并停止后续队列。
- 三处都显示当前正在处理的名称/镜头与进度；保留原有单项生成按钮和单项停止按钮。
- 不调用真实供应商，不改变后端 Provider 合同，不引入并发批量提交。

### 实施与验证清单

- [x] 为生成追踪器增加可等待终态的前端契约，供顺序队列复用。
- [x] 实现资产图逐项队列、停止和当前项状态。
- [x] 实现分镜图逐项队列、停止和当前项状态。
- [x] 实现视频逐项队列、停止和当前项状态，并与分镜图入口分离。
- [x] 生成终态统一由 `useAnnounceGenerationOutcomes` 投影，页面不再为队列散落成功/失败/停止提示。
- [x] 运行前端测试、lint、build；后端测试、typecheck、build。
- [x] 回填 history/README，记录未调用真实供应商。
- [x] 本轮不执行 commit、push 或关机。
