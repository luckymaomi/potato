...
- 模型列表不在前端或 Provider 描述符中写死。选择优先级是节点显式模型 → AI 配置页全局预设 → 自动选择；节点显式模型和全局预设都是固定选择，不得因本地能力判断静默切换。只有前两项都为空时才自动选择，并优先采用目录已明确确认兼容的模型，再回退到能力未知但没有明确冲突的模型。三类全局预设均可留空；保存时后端验证供应商已启用、已配置且模型真实存在于对应类型的实时目录。`config.yaml` 只保存供应商连接所需的固定配置，不保存模型选择。
- 媒体模型目录同时返回生成模式、最大参考图数量和画幅比例。空模式列表、`maxReferenceImages=null` 或 `aspectRatios=null` 表示供应商目录没有说明该项，不表示不支持；前后端允许用户把节点参数原样提交给供应商验证。只有非空模式/比例列表明确不包含请求值，或明确参考图上限小于请求数量时才在本地拒绝。未知比例模型的检查器保留节点当前比例并提供通用比例选项；自动择模仍优先能力已确认的候选。
